...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            left_y = y[:idx + 1]
            right_y = y[idx + 1:]
            left_pos = np.count_nonzero(left_y == 1)
            left_neg = np.count_nonzero(left_y == 0)
            right_pos = np.count_nonzero(right_y == 1)
            right_neg = np.count_nonzero(right_y == 0)
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset is None or feature_subset ==0:
